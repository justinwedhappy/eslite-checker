...
def collect_homepage_banners(page) -> list:
    ...
    if not banners:
        # 抓不到任何 banner 時,把當下的截圖與網頁原始碼存下來方便除錯
        # (常見原因:目標網站對雲端機房 IP 有反爬蟲機制,實際載入的頁面跟預期不同)
        print(f"[debug] 目前頁面網址: {page.url}")
        print(f"[debug] 頁面標題: {page.title()}")
        try:
            page.screenshot(path="debug_homepage.png", full_page=True)
            print("[debug] 截圖已存成 debug_homepage.png")
        except Exception as e:
            print(f"[debug] 截圖失敗: {e}")
        try:
            with open("debug_homepage.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            print("[debug] HTML 已存成 debug_homepage.html")
        except Exception as e:
            print(f"[debug] 存 HTML 失敗: {e}")
